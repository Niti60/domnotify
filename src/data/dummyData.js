export const dashboardStats = [
  { label: 'Domains tracked', value: '58', detail: 'Available domains in portfolio' },
  { label: 'Expiring soon', value: '4', detail: 'Within 30 days' },
  { label: 'SSL alerts', value: '2', detail: 'Certificates needing attention' },
  { label: 'Monthly budget', value: '$1,240', detail: 'Projected renewal spend' },
];

export const expiringDomains = [
  { domain: 'workspace.io', domainExpiresAt: '2025-06-14', daysLeft: 9, registrar: 'Namecheap' },
  { domain: 'brandhub.co', domainExpiresAt: '2025-06-28', daysLeft: 23, registrar: 'Cloudflare' },
  { domain: 'launchpad.dev', domainExpiresAt: '2025-07-03', daysLeft: 28, registrar: 'Squarespace Domains' },
  { domain: 'secureapp.com', domainExpiresAt: '2025-07-14', daysLeft: 39, registrar: 'GoDaddy' },
];

export const monitoringDomains = [
  { domain: 'workspace.io', registrar: 'Namecheap', domainExpiresAt: '2025-06-14', renewal: '$12.99', status: 'Available', lastChecked: '2 hours ago' },
  { domain: 'brandhub.co', registrar: 'Cloudflare', domainExpiresAt: '2025-06-28', renewal: '$14.50', status: 'Review', lastChecked: '6 hours ago' },
  { domain: 'launchpad.dev', registrar: 'Squarespace Domains', domainExpiresAt: '2025-07-03', renewal: '$19.00', status: 'Available', lastChecked: '1 day ago' },
  { domain: 'secureapp.com', registrar: 'GoDaddy', domainExpiresAt: '2025-07-14', renewal: '$18.99', status: 'Pending', lastChecked: '4 hours ago' },
];

export const sslCertificates = [
  { domain: 'workspace.io', issuer: 'Let’s Encrypt', sslExpiresAt: '2025-06-10', daysLeft: 5, status: 'Renew soon' },
  { domain: 'brandhub.co', issuer: 'DigiCert', sslExpiresAt: '2025-07-03', daysLeft: 28, status: 'Valid' },
  { domain: 'secureapp.com', issuer: 'Comodo', sslExpiresAt: '2025-05-20', daysLeft: -10, status: 'Expired' },
];

export const whoisData = {
  domain: 'secureapp.com',
  registrar: 'Namecheap, Inc.',
  registrant: 'Example Corp',
  email: 'admin@secureapp.com',
  nameservers: ['ns1.namecheap.net', 'ns2.namecheap.net'],
  created: '2019-05-10',
  expires: '2025-05-20',
  status: 'Active',
  dnssec: 'Unsigned',
};

export const watchlistDomains = [
  { domain: 'futurestack.ai', registrar: 'Namecheap', domainExpiresAt: '2025-11-12', status: 'Available', score: 91, price: '$2,900' },
  { domain: 'growthlabs.dev', registrar: 'Cloudflare', domainExpiresAt: '2025-06-14', status: 'Expiring soon', score: 84, price: '$1,650' },
  { domain: 'launchcode.app', registrar: 'Squarespace Domains', domainExpiresAt: '2025-07-03', status: 'Available', score: 78, price: '$740' },
  { domain: 'cloudpilot.io', registrar: 'Namecheap', domainExpiresAt: '2025-06-18', status: 'Expiring soon', score: 70, price: '$1,100' },
  { domain: 'motiongrid.co', registrar: 'GoDaddy', domainExpiresAt: '2025-05-10', status: 'Expired', score: 65, price: '$1,250' },
  { domain: 'stackverse.ai', registrar: 'Cloudflare', domainExpiresAt: '2025-06-20', status: 'Expiring soon', score: 80, price: '$1,400' },
];

export const pricingPlans = [
  {
    name: 'Starter',
    price: '$0',
    description: 'Domain health tracking for small portfolios',
    features: ['Up to 15 domains', 'Basic monitoring', 'WHOIS lookup', 'Email reminders'],
    highlight: false,
  },
  {
    name: 'Pro',
    price: '$29',
    description: 'All core monitoring features for founders and agencies',
    features: ['Up to 100 domains', 'SSL & uptime alerts', 'Watchlist tracking', 'Priority support'],
    highlight: true,
  },
  {
    name: 'Agency',
    price: '$79',
    description: 'Scale monitoring for teams and domain portfolios',
    features: ['Unlimited domains', 'Custom alerts', 'CSV export', 'Team-ready tools'],
    highlight: false,
  },
];

export const registrarCards = [
  {
    name: 'Namecheap',
    price: '$13.50/yr',
    bestFor: 'Portfolio management',
    features: ['WHOIS privacy included', 'DNS management', 'Renewal alerts'],
    popularity: 'Best value',
    url: 'https://www.namecheap.com/',
    supportEmail: 'support@namecheap.com',
  },
  {
    name: 'Cloudflare Registrar',
    price: '$12.99/yr',
    bestFor: 'Low renewal costs',
    features: ['Free DNS', 'Fast propagation', 'Privacy protection'],
    popularity: 'Budget-friendly',
    url: 'https://www.cloudflare.com/products/registrar/',
    supportEmail: 'registrar-abuse@cloudflare.com',
  },
  {
    name: 'Squarespace Domains',
    price: '$15.00/yr',
    bestFor: 'Simple pricing',
    features: ['Clean UI', 'Privacy protection', 'Squarespace ecosystem'],
    popularity: 'Trusted option',
    url: 'https://domains.squarespace.com/',
    supportEmail: 'support@squarespace.com',
  },
];

export const domainSearchResults = [
  { domain: 'startupflow.com', available: true, price: '$9.99/yr', registrar: 'Namecheap' },
  { domain: 'appstream.io', available: true, price: '$29.99/yr', registrar: 'Cloudflare' },
  { domain: 'launchpad.dev', available: false, price: 'Taken', registrar: 'Owned' },
  { domain: 'brandtrack.ai', available: true, price: '$34.99/yr', registrar: 'Squarespace Domains' },
];

export const recentActivity = [
  { id: 1, message: 'SSL expiration alert created for secureapp.com', time: '2h ago' },
  { id: 2, message: 'WHOIS lookup completed for growthlabs.dev', time: 'Yesterday' },
  { id: 3, message: 'Domain workspace.io renewed for 1 year', time: '3 days ago' },
];

export const searchTlds = ['.com', '.io', '.dev', '.app', '.co', '.ai'];
